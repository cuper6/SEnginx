  ...
  */
  for (i = 0; ctx->special_scores && i < ctx->special_scores->nelts; i++) {
	...
	/* senginx */
        ...
        } else if (!ngx_strcmp(sc[i].sc_tag->data, "$LIBINJECTION_XSS")) {
            slot = NGX_HTTP_STATS_ATTACK_LIBINJECTION_XSS;
        } else if (!ngx_strcmp(sc[i].sc_tag->data, "$LIBINJECTION_SQL")) {
            slot = NGX_HTTP_STATS_ATTACK_LIBINJECTION_SQL;
        } else {
