...
static char *
ngx_http_naxsi_whitelist(ngx_conf_t *cf, ngx_command_t *cmd, void *conf); /* senginx */
...
/* senginx ip blacklist */
  ...
  if (ngx_http_wl_check_whitelist(r, &cf->whitelist) == NGX_OK) { /* senginx */
        return NGX_DECLINED;
  }
...
{
    ngx_http_naxsi_loc_conf_t *lcf = conf;
